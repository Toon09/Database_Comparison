...
class insertData():
    ...
    # MongoDB does not support the Decimal datatype, so it needas to be removed of all the data (replaced by float)
    # this function replaces all Decimal's ny floats
    ...
    # file msut be a json and in the same format as
    def storeFile(self, dataFile):
        ...
        with open(dataFile, "r") as file:
            ...
            for obj in jsonF:
                # exhaustively check all fields to check if theresa single decimal
                self.checkForDecimal(obj) # Decimal type breaks db, so repalced by float
                self.changeTimeFormat(obj) # time as string doesnt allow for good searches, so replaced by datetime (from micro-s to micro-s)
                self.shoreline.insert_one(obj)
    ...
    # cahnges the time from a string to a datetime type which is better used for ranges, etc
    def changeTimeFormat(self, doc):
        if "metadata" in doc: #
            ...
            #self.shoreline.update_one({'_id':doc['_id']}, {'$set':{"metadata.time":newTime}}) # code used ot be ran after all insertions not during